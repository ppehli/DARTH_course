...
owsa_nmb  <- run_owsa_det(params_range    = df_params_owsa,   # data.frame with parameters for OWSA
                          ...
                          n_wtp           = 120000,
                          verbose         = TRUE)            # extra argument to pass to FUN
```
...
calculate_ce_out(l_params_all, verbose = TRUE) 
...
for (i in 1:n_sim) { # i <- 1
  ...
  l_psa_input <- update_param_list(l_params_all, 
                                   data.frame(
                                      name  = colnames(df_psa_input),
                                      value = unlist(df_psa_input[i, ], use.names = FALSE)
                                    )
                                   )
  # Outcomes
  l_out_ce_temp  <- calculate_ce_out(l_psa_input, verbose = TRUE)
...
While the CEAD and efficiency frontier only show the probability of being cost effective, it says nothing about how good or bad a strategy is compared to the other strategy in case it is not cost-effective. 
...
In health economics, decisions must often be made under uncertainty for example, implementation of treatment AB or not. With VOI analysis we can quantify the value of reducing this uncertainty. It tells us whether investing in more data (e.g., further clinical trials or research) is worth the cost, before committing to a healthcare strategy. So VOI analysis doesn't tell us what decision to make, it tells us how confident we can be in our decision, and whether it is worth reducing uncertainty through more research. It is a buy in approach. 
```{r}
